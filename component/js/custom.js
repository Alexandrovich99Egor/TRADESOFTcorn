...
let discountImpurityPercentOut = document.querySelector('.percent-discount-int');
let discountImpurityHrivnaTotalOut = document.querySelector('.total-discount-int');
...
function calculateDiscount() {
    ...
    if (!isNaN(humidity) && !isNaN(discountPercentage)) {
        let Y = 150;
        let discount = (humidity - discountPercentage) * Y * (100 - discountPercentage) / (100 - discountPercentage);
    ...
    // Скидка в %
    if (!isNaN(humidity) && !isNaN(discountPercentage)) {
        let discountPercentResult = (humidity - discountPercentage) / (100 - discountPercentage) * 100;
        if (discountPercentResult < 0) {
            discountPercentResult = 0

        }

        discountResultPercentOut.innerHTML = `${discountPercentResult.toFixed(2)}`;
        calculateDiscountImpurity(discountPercentResult)

    }
    // если значений нету
    humidity == '' ? inpEnterHumidity.style.border = '1px solid red' :
...
function CountDiscountImpurity(FirstValue) {
    ...
    if (FirstValue !== null && FirstValue > 0) {
        ...
        discountImpurityHrivnaTotalOut.innerHTML = `${totalDiscount.toFixed(2)}грн.`;
    }
}
...
function calculateDiscountImpurity() {
    ...
    if (!isNaN(inpEnterImpurityValue) && !isNaN(discountPercentage)) {
        ...
        discountValueImpurity = ((inpEnterImpurityValue - discountPercentage)) * Y * ((100 - discountPercentage)) / (100 - inpEnterImpurityValue);

        console.log(discountValueImpurity);

        if (discountValueImpurity < 0) {
            discountValueImpurity = 0;
        }

        discountImpurityHrivnaOut.innerHTML = `${discountValueImpurity.toFixed(2)}`;

        // перадал в фукнцию значиение грн что бы посчитать
        CountDiscountImpurity(discountValueImpurity);
    }
    if (!isNaN(inpEnterImpurityValue) && !isNaN(discountPercentage)) {
        let discountPercentImpurity = (inpEnterImpurityValue - discountPercentage) / (100 - discountPercentage) * 100;

        if (discountPercentImpurity < 0) {
            discountPercentImpurity = 0;
        }

        discountImpurityPercentOut.innerHTML = `${discountPercentImpurity.toFixed(2)}%`
    }


    inpEnterImpurity.value == '' ? inpEnterImpurity.style.border = '1px solid red' :
...
// стиль имупта если не введено значени
